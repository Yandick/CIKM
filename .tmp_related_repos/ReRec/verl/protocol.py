...
class DataProto:
    ...
    def copy(self) -> "DataProto":
        """
        Create a deep copy of the current DataProto object.
        
        Returns:
            DataProto: A new DataProto object containing deep copies of all data
        """
        # Copy batch (TensorDict)
        if self.batch is not None:
            new_batch = TensorDict(
                source={k: v.clone() for k, v in self.batch.items()},
                batch_size=self.batch.batch_size
            )
        else:
            new_batch = None
        
        # Copy non_tensor_batch (numpy arrays in dictionary)
        new_non_tensor_batch = {
            k: v.copy() for k, v in self.non_tensor_batch.items()
        }
        
        # Copy meta_info (use deep copy to ensure nested dictionaries are also copied)
        new_meta_info = copy.deepcopy(self.meta_info)
        
        # Create new DataProto instance
        return type(self)(
            batch=new_batch,
