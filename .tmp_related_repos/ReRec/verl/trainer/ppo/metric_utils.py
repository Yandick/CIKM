...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    """
    # Get mask for response part
    max_response_length = batch.batch["responses"].shape[-1]
    response_mask = batch.batch["attention_mask"][:, -max_response_length:].bool()

    # Set invalid positions to a sufficiently small value (like -inf) so max won't select them
    token_level_scores = batch.batch["token_level_scores"].clone()
    token_level_rewards = batch.batch["token_level_rewards"].clone()
    
    # Use mask processing
    token_level_scores.masked_fill_(~response_mask, float('-inf'))
    token_level_rewards.masked_fill_(~response_mask, float('-inf'))
    
    # Then take max
    sequence_score = token_level_scores.max(-1).values
    sequence_reward = token_level_rewards.max(-1).values
